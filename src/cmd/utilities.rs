...
impl QoveryCommand {
    ...
    where
        STDOUT: FnMut(Result<String, Error>),
        STDERR: FnMut(Result<String, Error>),
    {
...
mod tests {
    ...
    fn test_binary_exist() {
        ...
        assert_eq!(does_binary_exist("/usr/bin/bash"), true);
    }

    #[test]
    fn test_run_version_for_command() {
        let ret = run_version_command_for("/usr/bin/ls");
